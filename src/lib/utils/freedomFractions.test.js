...
import { freedomFractions, cycleFractions } from '$lib/utils/freedomFractions';

test('calling without arguments should return undefined', () => {
	expect(freedomFractions.parse()).toEqual(undefined);
});

test('numeric value limited to four decimal places', () => {
	expect(freedomFractions.parse(1.25000087).numeric).toEqual(1.25);
});

test('numeric value limited to four decimal places', () => {
	expect(freedomFractions.parse(1.0625087676).numeric).toEqual(1.0625);
});

test('numeric value limited to four decimal places', () => {
	expect(freedomFractions.parse(106.06250983).fixed).toEqual('106.0625');
});

test('numeric value trimmed to feet and inches', () => {
	const { foot, inch } = freedomFractions.parse(106.6250983);
