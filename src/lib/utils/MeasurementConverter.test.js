...
import { convert } from './MeasurementConverter';

convert.options({ feet: false, caching: false });

test('parser respects 1" precision option', () => {
	const options = { precision: 1 };
	const { fraction } = convert.parse('1 1/4"', options);
	expect(fraction).toEqual('');
});

test('measurement respects 1" precision option', () => {
	const options = { precision: 1 };
	const { fraction } = convert.fromDecimalInches(1.25, options);
	expect(fraction).toEqual('');
});

test('parser respects feet option', () => {
	const { inches, feet, readable } = convert.parse('54"', { feet: false });
...
test('conversion respects feet option', () => {
	const { inches, feet, readable } = convert.fromDecimalInches(54);
	expect(feet).toEqual(0);
	expect(inches).toEqual(54);
	expect(readable).toEqual('54"');
});
...
test('parser should handle combinations of feet and inches greater than 12', () => {
	const { feet, inches, readable } = convert.parse('12\' 28 1/2"', { feet: true });
...
test('parser should ignore commas', () => {
	const { feet, inches, fraction, readable } = convert.parse('5,000\', 0, 1/2"', {
		feet: true
	});
...
test('invalid inputs should throw errors', () => {
	...
	invalidInputs.forEach((input) => {
		try {
			expect(convert.parse(input)).toThrowError('invalid');
		} catch (err) {
			// do nothing
		}
	});
...
test('parser respects feet option', () => {
	...
	const { feet, inches, readable } = convert.parse('5\' 10 1/2"', options);
	expect(feet).toEqual(0);
...
test('conversion respects commas option', () => {
	...
	expect(convert.fromDecimalInches(5000 * 12 + 8, options).readable).toEqual('5000\' 8"');
});

test('conversion handles high numbers', () => {
	expect(
		convert.fromDecimalInches(5000 * 12 + 8, {
			feet: true
		}).readable
	).toEqual('5,000\' 8"');
});
...
test('parser respects zeros option', () => {
	const options = { feet: true, zeros: true };
	const { feet, inches, fraction, readable } = convert.parse("1'", options);
...
test('should correctly parse feet, inches and a fraction string', () => {
	const options = { feet: true, zeros: true };
	const { feet, inches, fraction } = convert.parse('4\' 3 1/2"', options);
...
test('should convert decimal inches to feet and inches', () => {
	const options = { feet: true, zeros: true };
	const { feet, inches } = convert.fromDecimalInches(13, options);
	expect(feet).toEqual(1);
...
test('should convert foot measurement into decimal inches', () => {
	const options = { feet: true, zeros: true };
	expect(convert.parse("1'", options).feet).toEqual(1);
});

test('should return undefined when called without arguments', () => {
	expect(convert.fromDecimalInches()).toEqual(undefined);
});

test('numeric value respects decimals option', () => {
	const options = { decimals: 4 };
	const { fixed, numeric } = convert.fromDecimalInches(1.25000087, options);
...
test('numeric value respects decimals option', () => {
	const options = { decimals: 4 };
	const { fixed, numeric } = convert.fromDecimalInches(1.0625087676, options);
