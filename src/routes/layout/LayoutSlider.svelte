<script lang="ts">
	...
	function handleTouchMove(event: TouchEvent) {
		...
		if ((newValue > value && direction < 1) || (newValue < value && direction > -1))
			origin = [x, originY];
	...
	function handleDrag(event: DragEvent) {
		...
		const deltaH = ((x - originX) as number) * rate;
		const deltaV = y - originY;
		const dynamicPrecision = precisionFromPixels(deltaV);
		// let newValue = +(value + change).toFixed(decimals);
		let newValue = Math.round((value + deltaH * (1 / precision)) * precision) / precision;
		if (newValue === value && dynamicPrecision === precision) return;
		if (newValue > max) newValue = max;
		if (newValue < min) newValue = min;
		if ((newValue > value && direction < 1) || (newValue < value && direction > -1))
