<script lang="ts">
	...
	function scrollToBottom(element) {
		backdrop.scroll({ top: backdrop.scrollHeight });
	}
</script>
	...
	<div class="calculator-body">
		...
		<div class="calculator-buttons">
			...
			{#if denomination}
				...
				>
			{:else}
				<button on:click={handleButtonPress} aria-label="0" id="0" style="grid-column: 1 / 3;"
					><span>0</span></button
				>
				<button on:click={handleButtonPress} aria-label="." id="." title="insert decimal point"
					><span class="inverted">.</span></button
				>
			{/if}
...
<style>
	...
	button span {
		...
		pointer-events: auto;
		/* border-radius: 12px; */
		border-radius: calc(var(--column-width) / 4);
		background: #fff;
		display: flex;
		align-items: center;
		justify-content: center;
